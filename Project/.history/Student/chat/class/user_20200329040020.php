...
class user{
	...
	function chat_history($user,$to_user){
		$sql = "SELECT * FROM chat_message WHERE (to_user_id=:to_user_id and from_user_id=:from_user_id) or (to_user_id=:from_user_id and from_user_id=:to_user_id) order by timestamp aesc";
		$query=$this->pdo->prepare($sql);
