...
if(isset($_POST["submit"]))
{
    ...
    if(!empty($_FILES["file"]["type"])){
        ...
        $vaild_exe=array("pdf", "doc","docx");
        $temporary=explode(".",$_FILES["file"]["name"]);
        $file_extension=end($temporary);
        if(($_FILES["file"]["type"]=="file/png")||($_FILES["file"]["type"]=="image/jpeg") ||($_FILES["file"]["type"]=="image/jpg") && in_array($file_extension,$valid_extensions))
         {
             $sourcePath=$_FILES["file"]['tmp_name'];
            move_uploaded_file($sourcePath,$upload_dir_img.$img_title);
            $hostImg='img/'.$img_title;
           
            $stm=$con->prepare("INSERT INTO final_report (file_data) VALUES (:hostImg)");
