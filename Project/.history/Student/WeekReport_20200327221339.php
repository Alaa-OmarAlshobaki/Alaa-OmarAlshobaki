...
?>
 ...
 $con = new mysqli("localhost","root","","traino");
     $st=$con->prepare("SELECT id_trainingadvisor FROM student WHERE id_student ='$id'");
     $st->execute();
     $rs=$st->get_result();
     while ($row=$rs->fetch_assoc())
     {
...
?>
            <input class="b1" type="text" value="<?php echo $row['id_trainingadvisor'] ?>" name="Supervisor_Name"/><br><br><?php }?>
            <br><br>
        </div>
