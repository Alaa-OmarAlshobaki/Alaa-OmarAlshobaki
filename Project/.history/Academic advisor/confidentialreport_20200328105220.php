...
<html>
    ...
    <body>
           ...
           <form  method="POST" action="confidentialreport.php">
        <input type="text" name="studentname"/>
        <input type="submit"value="studentname" name="s" class="btn-danger"/>
    </form>
  <?php
   if (isset($_GET["id_confidentialreport"])) {
        $con = new mysqli("localhost", "root", "", "traino");
        $st = $con->prepare("select*from confidentialreport where studentname=?");
        $st->bind_param("s", $_POST["studentname"]);
        $st->execute();
        $rs = $st->get_result();
