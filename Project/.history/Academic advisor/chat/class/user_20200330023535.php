...
class user{
	...
	function user_name($user_id){
		$sql = "SELECT * FROM academicadvisor where academicid=:user_id";
	...
	function login_check($email,$password){
		$sql = "SELECT * FROM academicadvisor WHERE email=:email and password=:password";
		$query= $this->pdo->prepare($sql);
		$query->bindParam(':email',$email,PDO::PARAM_STR);
		$query->bindParam(':password',$password,PDO::PARAM_STR);
		$query->execute();
		$result=$query->fetch(PDO::FETCH_ASSOC);
		$_SESSION['academicid']=$result['academicid'];
		return $result['academicid'];
	}
