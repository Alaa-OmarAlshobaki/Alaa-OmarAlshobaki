           ...
           class="btn-danger"/><br></div>     
            ...
            if (isset($_POST["save"])) {
                ...
                if ($con->connect_error) {
                    ...
                    $st=$con->prepare("insert into student(serialNumber,name,studentid,major,personalnumber,homenumber,email,password) values(?,?,?,?,?,?,?,?)");
                    $st->bind_param("siss",$_POST["serialNumber"], $_POST["studentName"],$_POST["ID"],
                            $_POST["major"],$_POST["personalnumber"],$_POST["homenumber"],$_POST["email"],randomPassword());
                    $st->execute();
