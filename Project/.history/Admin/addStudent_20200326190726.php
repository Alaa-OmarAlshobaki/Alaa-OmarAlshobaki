           ...
           class="btn-danger"/><br></div>     
            ...
            if (isset($_POST["s"])) {
                ...
                if ($con->connect_error) {
                    ...
                    $stm1 = $con->prepare('select  phone FROM trainingadvisor WHERE increment = "'.$supervisorPhone.'"');
                    $stm1->execute();
                    $rs1 = $stm1->get_result();
                    ...
                    if ($rs1->num_rows >0){
                        ...
                        $stmInside = $con->prepare('select  status FROM insidejordan WHERE increment = "'.$id_inc.'"');
                        $stmInside->execute();
                        $rsInside = $stmInside->get_result();
                   
                        if ($rsInside->num_rows >0){
                            $
                            $stmUpdateInside = $con->prepare('UPDATE `insidejordan` SET  status=:firstName1
