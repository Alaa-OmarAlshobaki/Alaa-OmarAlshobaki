            ...
            if (isset($_POST["s"])) {
                ...
                if ($con->connect_error) {
                        ...
                        ,'".$_POST["supervisorPhone"]."'
                         ...
                         if ($con->query($sql) === TRUE ) {
                            ...
                            sendmail($_POST["supervisorEmail"] ,"The student is registered for training. You have the student's name:".$_POST["studentName"]."
                               ...
                               echo $id_inc;
                               $status="accept";UPDATE insidejordan SET status ='accept' WHERE increment=8
                               $sql2 ='UPDATE insidejordan SET (status) VALUES("'.$status.'") WHERE increment="'. $id_inc.'"'; 
                            
                                if ($con->query($sql2) === TRUE ) {
                                    echo "<script type= 'text/javascript'>alert('Error: " . $sql2 . "<br>" . $con->error."');</script>";
                    ...
                    sendmail($_POST["supervisorEmail"] ,"The student is registered for training. You have the student's name:".$_POST["studentName"]."
                       ...
                       echo $id_inc;
                       $status="accept";
                       $q_ins='UPDATE `insidejordan` SET  status=:status WHERE increment = "'.$id_inc.'"'; 
                       $ins=$con->prepare($q_ins);
                       $res=$ins->execute();
                    } else {
                    echo "<script type= 'text/javascript'>alert('Error: " . $sql . "<br>" . $con->error."');</script>";
                    }
