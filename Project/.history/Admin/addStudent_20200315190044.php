...
<html>
    ...
    <?php
   $idCompany= isset($_GET['increment']) && is_numeric($_GET['increment'])?intval($_GET['increment']): 0;
   $stmEditRecVou=$con->prepare("SELECT * FROM insidejordan
                                       
                                           WHERE increment=:idCompany");
   $stmEditRecVou->bindParam(':idCompany', $idCompany);
   $stmEditRecVou->execute();
   if($stmEditRecVou->rowCount()>0){
       $rowEditRecVou=$stmEditRecVou->fetch(PDO::FETCH_ASSOC);
       $_SESSION['id']=$rowEditRecVou['id'];
