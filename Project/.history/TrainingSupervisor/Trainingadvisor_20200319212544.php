...
$st = $con->prepare("select*from insidejordan where serialNumber=?");
$st->bind_param("i", $_POST["serialNumber"]);
$st->execute();
$rs = $st->get_result();
while ($row = $rs->fetch_assoc()) {
    
                                            $con = new mysqli("localhost", "root", "", "traino");
                                            $stm = $con->prepare("select*from insidejordan where serialNumber=?");
                                       $stm=$con->prepare('SELECT *
                                                                    FROM `student`
                                                                 ');
                                       $stm->execute();
                                       if($stm->rowCount() >0){
                                         while($rowSele=$stm->fetch(PDO::FETCH_ASSOC)){
                                           
                                             extract($rowSele);
                                             $idStu=$rowSele['id_student'];
                                             $StuNum=$rowSele['studentid'];
