                    ...
                    <table class="table table-dark table-hover text-center" style="padding:8px;text-align:center;">
                        ...
                        <tbody>
                            ...
                            <tr>
                               ...
                               <td style="padding-top:60px"> <select class="custom-select" style="width:50% ; float:right!important"autocomplete="on"  id="inlineFormCustomSelect" name="receFrom" required="required">
                                <option selected> اختار من ...</option>
                                           <?php
                                       $stmSelectCom=$con->prepare('SELECT *
                                                                    FROM `company`
                                                                 ');
                                       $stmSelectCom->execute();
                                       if($stmSelectCom->rowCount() >0){
                                         while($rowSeleCom=$stmSelectCom->fetch(PDO::FETCH_ASSOC)){
                                           
                                             extract($rowSeleCom);
                                             $idCom=$rowSeleCom['company_id'];
                                             $nameOfCom=$rowSeleCom['company_name'];
                                        

                                             ?>
                                           
                                <option  value="<?php echo $idCom;?>" ><?php echo $nameOfCom;?></option>
                        ...
                        if (isset($_POST["s"])) {
                            ...
                            if ($con->connect_error) {
                                ...
                                $sql = "INSERT INTO attendance (id_student, attendance_date,sunday,monday,Tuesday,Wednesday,Thursday,numHours)
                                VALUES ('".$_POST["id"]."'
                                ,'".$_POST["date"]."'
                                ,'".$_POST["Sun"]."'
                                ,'".$_POST["Mon"]."'
